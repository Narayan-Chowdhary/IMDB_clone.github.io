...
var searchListDisplay = document.getElementsByClassName('fav-container');

fetch(`http://www.omdbapi.com/?i=tt3896198&apikey=${key}`)
    .then(res => res.json())
    .then(data => console.log(data));

// om keypress - movie search function initiate
inputSearch.addEventListener('input', movieFind);
...
async function oneMovie() {
    ...
    `
    // Appending the output into the HTML
...
async function displayMovies(movies) {
    var output = '';
    //moving over the movies list which is passed as an argument to our function
    for (i of movies) {
...
async function movieFind() {
    const url = `https://www.omdbapi.com/?s=${(inputSearch.value).trim()}&page=1&apikey=${key}`
    const res = await fetch(`${url}`);
    const data = await res.json();
...
async function favoritesMovieLoader() {
    ...
    var output = ''
    //moving over all the movies in the localstorage
    for (i in localStorage) {
            ...
            <div class="fav-details">
                <div class="fav-details-box">
                    ...
                    <div style="color: maroon">
                        <i class="fa-solid fa-trash" style="cursor:pointer;" onClick=removeFavorite('${Id}')></i>
                    </div>
                </div>
            </div>
        </div>
